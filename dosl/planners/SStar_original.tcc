...
#ifndef __DOSL_S_STAR_original_TCC
#define __DOSL_S_STAR_original_TCC
// user-readable
#define DOSL_ALGORITHM_SStar_original
...
// --------------------------------------------------------------------
class SStar_original
{
public:
    declare_alg_name("SStar_original"); // macro from '_planner_bits'
    ...
    public:
        ...
        LineageDataType next_generation(void)
        {
            LineageDataType ret(*this);
            ++(ret.generation);
            return (ret);
        }
    };
    ...
    public:
        ...
        virtual NodeType operator+(const NodeType &b) const
        { // n1 + n2 == n1
            _dosl_default_fun_warn("SStar::Node::operator+");
            return (*((NodeType *)this));
        }
        virtual NodeType operator*(const DoubleType &c) const
        { // n1 * c  == n1 (right scalar multiplication)
            _dosl_default_fun_warn("SStar::Node::operator*");
            return (*((NodeType *)this));
        }
    };
...
// Constants
        ...
        {
            _dosl_default_fun_warn("SStar::Algorithm::getStartNodes");
...
#if _DOSL_EVENTHANDLER
        inline void nodeEvent(NodeType &n, unsigned int e)
        {
            n.nodeEvent(e);
        }
#endif
        inline virtual void print(NodeType &n, std::string head)
        {
            n.print(head);
        }
        // Derived functions. Can also be directly overwritten.
...
SStar_original::Algorithm<AlgDerived, NodeType, CostType>::Algorithm(AlgDerived *shared_instance_p) : _this(static_cast<AlgDerived *>(this))
{
...
template <class AlgDerived, class NodeType, class CostType>
void SStar_original::Algorithm<AlgDerived, NodeType, CostType>::generate_successors(NodeType *node_in_hash_p)
{
    if (!(node_in_hash_p->successors_created)) // successors were not generated previously
    {
        ...
        for (a = 0; a < this_successors.size(); a++)
        {
            ...
            // set data that may need to be accessed from operator==
            this_successors[a].lineage_data = node_in_hash_p->lineage_data;
...
template <class AlgDerived, class NodeType, class CostType>
void SStar_original::Algorithm<AlgDerived, NodeType, CostType>::CheckAndPushNodeIntoHeap(NodeType *node_in_hash_p, bool heapKeyUpdated,
                                                                                         bool cascadeToChildren)
{
    node_in_hash_p->expanded = false;

    if (node_in_hash_p->heap_pos < 0)
    { // may already in heap due to earlier initiated backtracking
...
template <class AlgDerived, class NodeType, class CostType>
void SStar_original::Algorithm<AlgDerived, NodeType, CostType>::search(void)
{
    ...
    for (a = 0; a < start_nodes.size(); a++)
    {
        ...
        // push in heap
        if (thisNodeInHash_p->heap_pos < 0)
        { // should not push the same node twice into heap
    ...
    while (!(node_heap_p->empty()))
    {
            ...
            if (progress_show_interval > 0)
            {
                if (expand_count % progress_show_interval == 0)
                {
                    _dosl_printf("Number of states expanded: %d. Number of simplices ceated: %d. Node heap size: %d."
                                 "Time elapsed: %f_score s.",
                                 expand_count, AllSimplices.size(), node_heap_p->size(), timer.read());
                }
            }
        expand_count++;
        ...
        for (auto it = thisNodeInHash_p->successors.begin(); it != thisNodeInHash_p->successors.end(); ++it)
        {
            ...
            if (_dosl_verbose_on(1))
            {
                ...
                (this_neighbour_node_in_hash_p->g_score == std::numeric_limits<double>::max()) ? printf("INF\n") : printf("%f\n", this_neighbour_node_in_hash_p->g_score);
            }

            // compare & update
            if (neighborJustCreated)
            { // it's a 1-simplex for sure
            ...
            else if (this_neighbour_node_in_hash_p->g_score - test_g_val > _MS_DOUBLE_EPS)
            {
                ...
                if (isTestingForUnexpanding)
                { // Previously expanded. Need to backtack since g-score is updated
                    // Unexpand
...
void SStar_original::Node<NodeType, CostType>::clear_search_data(unsigned int mode)
{ // reset everything other than successor list
    post_hash_insert_initiated = false;
    expanded = false; // came_from = NULL;
    heap_pos = -1;
    g_score = (CostType)0.0; // f_score = (CostType)0.0;
    if (mode & CLEAR_NODE_SUCCESSORS)
    {
        successors.clear();
        successors_created = false;
    }
    if (mode & CLEAR_NODE_LINEAGE)
        lineage_data = LineageDataType();
    backTrackVertex = 0;
    CameFromSimplex = NULL;
}

// -------------------------------------------------------------------------------------

template <class AlgDerived, class NodeType, class CostType>
void SStar_original::Algorithm<AlgDerived, NodeType, CostType>::clear(unsigned int mode)
{
    NodeType *thisNodeInHash_p;
    if (mode & CLEAR_NODES_HEAP)
        node_heap_p->clear();
    /* while (!(node_heap_p->empty()))
        node_heap_p->pop(); */
...
SStar_original::Algorithm<AlgDerived, NodeType, CostType>::reconstruct_weighted_pointer_path(NodeType n) // n should be a node that is in the S-star hash
{
    _dosl_verbose_head(1);
    ...
    typename MetricSimplexContainerType::PathPointType path_pt =
        typename MetricSimplexContainerType::PathPointType(np->CameFromSimplex->w, np->CameFromSimplex->g_came_from_point, thisPt);

    if (_dosl_verbose_on(0))
        n.print("Reconstruct path called");
    if (_dosl_verbose_on(1))
        _dosl_printf("Node in hash: %x.", np);
...
SStar_original::Algorithm<AlgDerived, NodeType, CostType>::reconstruct_pointer_path(NodeType n, bool convertWeightsToInt)
{
