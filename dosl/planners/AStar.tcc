...
// Constants
        ...
        std::vector<std::unordered_map<NodeType *, CostType>> reconstruct_path_with_length(NodeType n, double path_length);
...
template <class AlgDerived, class NodeType, class CostType>
std::vector<std::unordered_map<NodeType *, CostType>>
AStar::Algorithm<AlgDerived, NodeType, CostType>::reconstruct_path_with_length(NodeType n, double path_length)
{
    std::vector<NodeType *> npVec = reconstruct_pointer_path(n);
    ...
    for (auto it = npVec.begin(); it != npVec.end(); ++it)
    {
        std::unordered_map<NodeType *, CostType> thisPt;
        if (it != npVec.begin())
            curr_length += (*it)->getDist(**(it - 1));
        if (curr_length > path_length)
            break;
        thisPt[(*it)] = 1.0;
        ret.push_back(thisPt);
    }
