...
#ifndef __DOSL_ThetaStar_original_TCC
#define __DOSL_ThetaStar_original_TCC
// user-readable
#define DOSL_ALGORITHM_ThetaStar_original
...
class ThetaStar_original
{
public:
    declare_alg_name("ThetaStar_original"); // macro from '_planner_bits'
...
// Constants
        ...
        {
            _dosl_default_fun_warn("ThetaStar::Algorithm::isSegmentFree");
            *c = (CostType)0.0;
            return (true);
        }
        ...
        {
            _dosl_default_fun_warn("ThetaStar::Algorithm::getStartNodes");
...
ThetaStar_original::Algorithm<AlgDerived, NodeType, CostType>::Algorithm(AlgDerived *shared_instance_p) : _this(static_cast<AlgDerived *>(this))
{
...
template <class AlgDerived, class NodeType, class CostType>
void ThetaStar_original::Algorithm<AlgDerived, NodeType, CostType>::generate_successors(NodeType *node_in_hash_p)
{
...
template <class AlgDerived, class NodeType, class CostType>
void ThetaStar_original::Algorithm<AlgDerived, NodeType, CostType>::search(void)
{
    ...
    while (!(node_heap_p->empty()))
    {
        ...
        for (auto it = thisNodeInHash_p->successors.begin(); it != thisNodeInHash_p->successors.end(); ++it)
        {
            ...
            if (_this->isSegmentFree(*(thisNodeInHash_p->came_from), *this_neighbour_node_in_hash_p, &grandparentSegmentCost))
            { // path 2
...
void ThetaStar_original::Node<NodeType, CostType>::clear_search_data(unsigned int mode)
{
    ...
    came_from = NULL;
    heap_pos = -1;
    g_score = (CostType)0.0;
    f_score = (CostType)0.0;
...
template <class AlgDerived, class NodeType, class CostType>
void ThetaStar_original::Algorithm<AlgDerived, NodeType, CostType>::clear(unsigned int mode)
{
    NodeType *thisNodeInHash_p;
...
template <class AlgDerived, class NodeType, class CostType>
std::vector<NodeType *> ThetaStar_original::Algorithm<AlgDerived, NodeType, CostType>::reconstruct_pointer_path(NodeType n)
{
    _dosl_verbose_head(1);
    ...
    if (_dosl_verbose_on(0))
        n.print("Reconstruct path called on ");
    if (_dosl_verbose_on(1))
        _dosl_printf("Node in hash: %x.", thisNodeInHash_p);
...
ThetaStar_original::Algorithm<AlgDerived, NodeType, CostType>::reconstruct_weighted_pointer_path(NodeType n)
{
