...
void binary_heap<key, CompareFunctor, HeapPosFunctor>::update(Key np, BubbleDirection bubdir)
{
    ...
    if (!(_in_heap(np)))
    {
        _dosl_warn("attempted to heap-update node not in heap");
        return;
    }
    int currentPos = HeapPosFunctorInstance(np);

    // Bubble up
    if (currentPos > 0 && bubdir != DOWN_BUBDIR)
    {                                         // either up or unknown
        int parentPos = (currentPos - 1) / 2; // position of parent
        if (_less_than(np, heapArray[parentPos]))
        { // parent is larger. bubble up
    ...
    if (_less_than(heapArray[exchangeChildPos], np))
    { // the smallest child is smaller than self. bubble down
