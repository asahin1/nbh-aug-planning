...
public:
    ...
    {
        if (HashTable)
            _dosl_err("Hash table already initiated!");
        hash_table_size = s;
    }
    ...
    template <class FindEqFunc = EqualToFunctor>
    std::vector<Key *> getall(key &n, FindEqFunc eqfunc_instance, bool search_other_bins = false);
    ...
    public:
        ...
        int bin, pos;
        forward_iterator(fast_unordered_set *pp, int bb = 0, int poss = -1) : p(pp), bin(bb), pos(poss)
        {
            if (pos < 0)
                operator++();
        }
        ...
        forward_iterator &operator++()
        {
            do
            {
                if (bin >= p->hash_table_size)
                {
                    bin = p->hash_table_size;
                    pos = 0;
                } // end
                else if (pos < p->HashTable[bin].size() - 1)
                    ++pos;
                else
                {
                    ++bin;
...
{
    if (HashTable)
        _dosl_err("Hash table already initiated! Call clear to reset and re-initiate.");
